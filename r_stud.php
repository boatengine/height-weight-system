...
<body style="background-color:#F6F8FF;">
    ...
    <?php
        ...
        if(isset($_POST['room_check'])) {
            ...
            $sql = "SELECT * FROM data_nn WHERE room = :room AND st1 = '' ORDER BY id ASC" ; //เรียกข้อมูลจากฐานข้อมูลมาใช้งานและนับข้อมูลในฐานข้อมูล
            $query = $conn->prepare($sql);
            $query->bindParam(':room', $room, PDO::PARAM_INT);
            $query->execute();
    ...
    ?>
      ...
      <section id="login" class="about section-padding">
        <div class="container shadow-lg p-5 ff1 rounded-4 bg-white ">
              ...
              <img src="img/h92.png" alt="" class="img-fluid img-save " width="10%" loading="eager">
                    <h2 class="text-center "><span class="text-danger"><b>ห้อง <?= $room ?> นักเรียนที่ยังไม่บันทึกข้อมูล</b></span></h2>
                    <div class="d-grid gap-2 col-3 mx-auto m-3">
                        <a href="index.php" class="btn btn-danger form-control mb-8 rounded-pill btn-sm"><i class='bx bx-right-arrow-alt'></i>&nbsp;กลับหน้าแรก</a>
                    </div>
                ...
                <div class="col-lg-12 col-md-12 col-12 mx-auto rounded-4 shadow p-5">
                    ...
                    <div class="table-responsive">    
                        <table class="table mt-3 table-hover display nowrap" id="myG" style="width:100%">
                            <thead class="p-2 mb-2 bg-danger text-white">
                                <tr>
                                    <th scope="col" class="bg-danger text-white">#</th>
                                    <th scope="col" class="bg-danger text-white" style="width:10%">เลขประจำตัว</th>
                                    <th scope="col" class="bg-danger text-white">ห้อง</th>
                                    <th scope="col" class="bg-danger text-white">เลขที่</th>
                                    <th scope="col" class="bg-danger text-white">ชื่อสกุล</th>
                                    <th scope="col" class="bg-danger text-white">สถานะ</th>
